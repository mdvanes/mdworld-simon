...
{
	getRssFeed : function(){
		var rssUrl = full_theme_path + "/functions/cache.php";

		//clear the content in the div for the next feed.
		$("#blogroll").empty();
		$("#blogroll").append("<div class=\"status\">Loading...</div>");
		//use the JQuery to put the results in to an argument for parsing in the inline 
		...
		$.get(rssUrl, function(d) {
			...
			$(d).find("item").each(function() {
				var cssClass = "rotated";
				//name the current found item this for this particular loop run
				var $item = $(this);
				// grab the post title
				var title = $item.find("title").text();
				// grab the post's URL
				// the link is correctly in find("link").text(); because now the content type of the RSS is correct
			...
			// Repeat the list of items to get at least 90 items
			var items = $(d).find("item").length;
			if( items > 0 )
			{
				var repeats = Math.ceil( 90 / items );
				for( var i = 0; i < repeats; i++ )
				{
					// Print the items to the screen
					$("#blogroll").append($(totalHtml));
				}
			}
		});
	},
	addStyleFile : function( filename )
	{
		var path = Drupal.settings.basePath + "themes/simon/style/" + filename;
	...
	isIe : function(){
		...
		return htmlNode[0].className.indexOf("ie") > -1;
	}
};

/********* Start Initialization ************/
// Initialize W3C Template Layout with jQuery
if( !simon.isIe() ) {
 ...
 * http://mdworld.nl
 */
var graph={Graph:function(a){this.name=a.name;this.wrapperId=a.wrapperId;this.canvas;this.context;this.dynamicCanvas;this.dynamicContext;this.innerWrapper;this.debug=document.getElementById(this.name+"_debug");this.xMargin=50;this.yMargin=40;this.xUnit=a.xUnit;this.yUnit=a.yUnit;this.xOffset=a.xOffset;this.yOffset=a.yOffset;this.xLabelY;this.yLabelX=5;this.xLabels=a.xLabels;this.yLabels=a.yLabels;this.lines=a.lines;this.rawLines;this.colors=["#3366cc","#dc3912"];this.defaultColor="#0f0";this.enableXTicks=a.enableXTicks;this.enableBullets=a.enableBullets;this.bulletRadius=4;this.x;this.y;this.enableBulletLabels=a.enableBulletLabels;this.minLabelWidth=30;this.maxLabelWidth=200;this.init=function(){if(this.debug!=null){this.debug.innerHTML="<h2>Debug is enabled</h2>"}this.createStaticCanvas();this.xLabelY=this.canvas.height-12;this.context=this.canvas.getContext("2d");if(this.enableBulletLabels){this.createDynamicCanvas();if(this.dynamicCanvas!=null){this.dynamicContext=this.dynamicCanvas.getContext("2d")}}this.drawXLabels(this.xLabels);this.drawYLabelsAndGuidelines(this.yLabels);this.rawLines=new Array(this.lines.length);for(var b=0;b<this.lines.length;b++){this.rawLines[b]=this.drawLine(this.lines[b],this.getColor(b))}};this.redraw=function(){this.reset();this.drawXLabels(this.xLabels);this.drawYLabelsAndGuidelines(this.yLabels);this.rawLines=new Array(this.lines.length);for(var b=0;b<this.lines.length;b++){this.rawLines[b]=this.drawLine(this.lines[b],this.getColor(b))}this.dynamicCanvas.onmousemove=null;var c=this;this.dynamicCanvas.addEventListener("mousemove",function(d){c.trackBulletLabel(d)},false)};this.createStaticCanvas=function(){var e=this.name+"_innerWrapper";var d=document.createElement("div");d.setAttribute("id",e);document.getElementById(this.wrapperId).appendChild(d);this.innerWrapper=document.getElementById(e);var b=this.name+"_static";var c=document.createElement("canvas");c.setAttribute("id",b);this.innerWrapper.appendChild(c);this.canvas=document.getElementById(b);this.canvas.setAttribute("width",this.canvas.clientWidth);this.canvas.setAttribute("height",this.canvas.clientHeight)};this.createDynamicCanvas=function(){var c=this.name+"_dynamic";var b=document.createElement("canvas");b.setAttribute("id",c);b.setAttribute("width",this.canvas.width+this.maxLabelWidth);b.setAttribute("height",this.canvas.height);this.innerWrapper.appendChild(b);this.dynamicCanvas=document.getElementById(c);var d=this;this.dynamicCanvas.addEventListener("mousemove",function(e){d.trackBulletLabel(e)},false)};this.trackBulletLabel=function(e){if(this.dynamicCanvas==null){alert("No canvas is available to draw bullet labels.");return}this.getMousePosition(e);lines:for(var d=0;d<this.rawLines.length;d++){var b=this.rawLines[d];for(var c=0;c<b.length;c++){var f=b[c];if((this.x>f.x-this.bulletRadius)&&(this.x<f.x+this.bulletRadius)&&(this.y>f.y-this.bulletRadius)&&(this.y<f.y+this.bulletRadius)){if(this.debug!=null){this.debug.innerHTML="x: "+this.x+"  y: "+this.y+"<br/>canvas offsetLeft="+this.canvas.offsetLeft+"<br/>innerWrapper offsetLeft="+this.innerWrapper.offsetLeft+"<br/> label="+f.label}this.dynamicCanvas.width=this.dynamicCanvas.width;this.drawBulletLabel(f.x,f.y,f.label);break lines}else{this.dynamicCanvas.width=this.dynamicCanvas.width}}}};this.drawBulletLabel=function(c,h,d){this.dynamicContext.fillStyle="#fff";this.dynamicContext.strokeStyle="#000";this.dynamicContext.lineWidth=1;this.dynamicContext.beginPath();var e=Math.round(h);var g=Math.round(c);var b=this.minLabelWidth;var f=this.dynamicContext.measureText(d).width;if(d==undefined){return}else{if(f>this.maxLabelWidth){b=this.maxLabelWidth;d=this.getFittingLabel(this.maxLabelWidth,d)}else{if(f>this.minLabelWidth){b=f}}}this.dynamicContext.moveTo(g-10.5,e+24.5);this.dynamicContext.lineTo(g-5.5,e+24.5);this.dynamicContext.lineTo(g+0.5,e+19.5);this.dynamicContext.lineTo(g+5.5,e+24.5);this.dynamicContext.lineTo(g+b+0.5,e+24.5);this.dynamicContext.lineTo(g+b+0.5,e+24.5);this.dynamicContext.lineTo(g+b+0.5,e+42.5);this.dynamicContext.lineTo(g-10.5,e+42.5);this.dynamicContext.lineTo(g-10.5,e+24.5);this.dynamicContext.fill();this.dynamicContext.stroke();this.dynamicContext.closePath();this.dynamicContext.fillStyle="#000";this.dynamicContext.fillText(d,c-5,h+37)};this.getFittingLabel=function(c,b){var d=this.dynamicContext.measureText(b).width;if(d>c){b=b.substr(0,b.length-1);return this.getFittingLabel(c,b)}else{return b}};this.getMousePosition=function(d){var e=0;var c=0;var b=this.canvas;do{e+=b.offsetLeft;c+=b.offsetTop}while(b=b.offsetParent);this.x=d.pageX-e;this.y=d.pageY-c};this.getColor=function(b){if(b>(this.colors.length-1)){return this.defaultColor}else{return this.colors[b]}};this.drawXLabels=function(c){for(var b=0;b<c.length;b++){this.drawXLabel(c[b].x,c[b].text)}};this.drawXLabel=function(b,d){var c=(this.xMargin)+((b-this.xOffset)*this.xUnit);this.drawRawXLabel(c,d)};this.drawRawXLabel=function(b,c){if(this.enableXTicks){this.context.moveTo(b+0.5,this.xLabelY-27);this.context.lineTo(b+0.5,this.xLabelY-25);this.context.stroke()}this.context.save();this.context.beginPath();this.context.translate(b,this.xLabelY);this.context.rotate(-0.5);this.context.fillText(c,0,0);this.context.restore()};this.drawYLabelsAndGuidelines=function(c){this.drawYLabelAndGuideline(c[0].y,c[0].text,true);for(var b=1;b<c.length;b++){this.drawYLabelAndGuideline(c[b].y,c[b].text,false)}};this.drawYLabelAndGuideline=function(e,d,c){var b=this.canvas.height-this.yMargin-((e-this.yOffset)*this.yUnit);b=b+0.5;this.drawRawYLabelAndGuideline(b,d,c)};this.drawRawYLabelAndGuideline=function(d,c,b){this.context.save();this.context.beginPath();this.context.fillText(c,this.yLabelX,d+3);this.context.restore();this.context.save();if(b){this.context.strokeStyle="#000"}else{this.context.strokeStyle="#eee"}this.context.moveTo(this.xMargin,d);this.context.lineTo(this.canvas.width-5,d);this.context.stroke();this.context.restore()};this.drawLine=function(f,c){var e=new Array(f.length);for(var d=0;d<f.length;d++){var b=this.xMargin+((f[d].x-this.xOffset)*this.xUnit);var g=this.canvas.height-this.yMargin-((f[d].y-this.yOffset)*this.yUnit);e[d]={x:b,y:g,label:f[d].label}}this.drawLineRaw(e,c);return e};this.drawLineRaw=function(d,b){this.context.beginPath();this.context.moveTo(d[0].x,d[0].y);for(var c=1;c<d.length;c++){this.context.lineTo(d[c].x,d[c].y);if(this.exeedsDimensions(d[c])){return}}this.context.strokeStyle=b;this.context.lineWidth=2;this.context.stroke();if(this.enableBullets){this.drawBulletsRaw(d,b)}};this.drawBulletsRaw=function(d,b){for(var c=0;c<d.length;c++){this.context.beginPath();this.context.fillStyle=b;this.context.arc(d[c].x,d[c].y,this.bulletRadius,0,Math.PI*2,true);this.context.closePath();this.context.fill()}};this.exeedsDimensions=function(b){if(b.x>this.canvas.width){alert("Exceeds dimensions\ncanvas width="+this.canvas.width+" x="+b.x);return true}else{if(b.y>this.canvas.height){alert("Exceeds dimensions\ncanvas height="+this.canvas.height+" y="+b.y);return true}else{return false}}};this.reset=function(){this.canvas.width=this.canvas.width}}};;
